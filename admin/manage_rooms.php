...
if (isset($_POST['form1'])) {
  try {
    ...
    $statement = $db->prepare("insert into tbl_rooms (name, block) values(?,?)");
    $statement->execute(array($_POST['name'], $_POST['block']));
...
if (isset($_POST['form2'])) {
  try {
    ...
    $statement = $db->prepare("UPDATE tbl_rooms SET name=?,block=? where id=?");
    $statement->execute(array($_POST['name'],$_POST['block'], $_POST['hdn']));
